...
//    private int spectrumAmpOutArrayPt = 0;        // Pointer for spectrumAmpOutArray
    ...
    private void init(int fftlen, int _hopLen, int sampleRate, int minFeedSize, String wndName) {
        ...
        spectrumAmpOutDB = new double[fftlen/2+1];//in dB; spectrumAmpOutDB is used to calculate RMS
    ...
    void feedData(short[] ds, int dsLen) {
        ...
        while (dsPt < dsLen) {
            ...
            if (spectrumAmpPt == inLen) {    // enough data for one FFT
                ...
                fftToAmp(spectrumAmpOutTmp, spectrumAmpInTmp);//spectrumAmpOutTmp  == holds PSD buffer
...
//                spectrumAmpOutArrayPt = (spectrumAmpOutArrayPt+1) % spectrumAmpOutArray.length;
    ...
    {//this function corrects automatic gain control. I must run this function before I do any measurement in spectrumAmpOut
        if (nAnalysed != 0) {    // no new result
            ...
            System.arraycopy(sAOC, 0, spectrumAmpOut, 0, outLen);//now spectrumAmpOut is normalized by mic gain
    ...
    void calculatePeak() {
        //getSpectrumAmpDB();commented out by AV since it is already called inside Sampling Loop>run(). It can only be called once. It was a bug to call it again.
        ...
        // The peak amplitude around spectrumDB should look like quadratic curve after good window function
        // a*x^2 + b*x + c = y
        // a - b + c = x1
        //         c = x2
        // a + b + c = x3
    ...
    double getRMSFromFT(int freq) {
        ...
        double s = 0;
        int indexFreq=1;
        if(freq>0) indexFreq=(freq*fftLen) / sampleRate;//convert freq to indexFreq in freq domain: freq=sampleRate/2 corresponds to spectrumAmpOut.Length;
    ...
    {//
        ...
        {//scan for peak thickness that generates the highest ratio against background
            ...
            mean=( ArrayAvgFreq(freqOfPeak, -round(freq*2.4f), -round(freq*1.1f)) + ArrayAvgFreq(freqOfPeak, round(freq*1.1f), round(freq*2.4f)) )/2;//this must be away from harmonics
            if( mean!=0 ) {
                ratio = max / mean;
